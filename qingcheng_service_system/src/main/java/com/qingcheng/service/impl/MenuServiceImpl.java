...
public class MenuServiceImpl implements MenuService {
    ...
    public List<Map> findAllMenu() {
        List<Menu> menuList = findAll();//查询得到全部菜单列表
        return findMenuListByParentId(menuList,"0");//得到所有一级菜单
    }
    ...
    private List<Map> findMenuListByParentId(List<Menu> menuList, String parentId) {
        ...
        for (Menu menu : menuList) {
            if (menu.getParentId().equals(parentId)){//将所有一级菜单对象筛选出来
                HashMap map = new HashMap();
                map.put("path",menu.getId());
                map.put("title",menu.getName());
                map.put("icon",menu.getIcon());
                map.put("linkUrl",menu.getUrl());
                map.put("children",findMenuListByParentId(menuList,menu.getId()));//这里使用了递归调用
                mapList.add(map);
            }
        }
        return mapList;
