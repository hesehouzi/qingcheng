...
public class SkuServiceImpl implements SkuService {
    ...
    public void saveAllPriceToRedis() {
        ...
        if (!redisTemplate.hasKey(CacheKey.SKU_PRICE)){
            //如果不存在 那么从数据库中查询 并加载到数据库
            System.out.println("实现商品价格缓存预热");//从数据库中查询所有价格并全部加载到缓存
            List<Sku> skuList = skuMapper.selectAll();//查询到全部商品
            for (Sku sku : skuList) {
                //将查询出来的所有的sku进行筛选 筛选出状态正常的sku对象
                if ("1".equals(sku.getStatus())){
                    //将状态正常的每一个sku的价格 以skuId为键 price为值都保存到缓存中
    ...
    public Integer findPrice(String id) {
        //1.从缓存中查询价格
        return (Integer) redisTemplate.boundHashOps(CacheKey.SKU_PRICE).get(id);//通过skuId查询对应的价格
    }

    /**
     * 保存价格到缓存 根据skuId将价格存入缓存
     * @param id
     * @param price
     */
    ...
    public boolean deductionStock(List<OrderItem> orderItemList) {
        //1.检查是否可以扣减库存
        boolean isDeduction=true;//true：可以扣减；false：不能扣减
        for (OrderItem orderItem : orderItemList) {
            Sku sku = findById(orderItem.getSkuId());//获得sku对象
            if (sku==null){//sku对象不存在 不扣减
                isDeduction=false;
                break;
            }
            if (!"1".equals(sku.getStatus())){//sku状态不为1 不扣减
                isDeduction=false;
                break;
            }
            if (sku.getNum().intValue()<orderItem.getNum().intValue()){//数据库中对应的sku库存数量<购物车中对应的购买数量 不扣减
                isDeduction=false;
                break;
            }
        }
        //2.执行扣减
        //2.1 isDeduction=true执行扣减
        if (isDeduction){
            //先对orderItemList进行遍历
            for (OrderItem orderItem : orderItemList) {
                skuMapper.deductionStock(orderItem.getNum(),orderItem.getSkuId());//扣减库存
