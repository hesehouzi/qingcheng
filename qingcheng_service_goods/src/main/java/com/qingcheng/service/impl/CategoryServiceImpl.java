...
public class CategoryServiceImpl implements CategoryService {
    ...
    public void add(Category category) {
        ...
        saveCategoryTreeToRedis();//数据修改之后重新进行缓存预热
    }
    ...
    public void delete(Integer id) {
        //先判断是否存在下级id 如果存在则抛出异常
        Example example = new Example(Category.class);
        Example.Criteria criteria = example.createCriteria();
        criteria.andEqualTo("parentId",id);//id和parentId相同的
    ...
    public List<Map> findCategoryTree() {
        ...
        return (List<Map>) redisTemplate.boundValueOps(CacheKey.CATEGORY_TREE).get();//强转成List<Map>结构
        //通过条件查询“isShow”的值为1的分类对象 先创建查询条件
        /*Example example = new Example(Category.class);
        Example.Criteria criteria = example.createCriteria();
    ...
    private List<Map> findByParentId(List<Category> categoryList,Integer parentId){
        ...
        for (Category category : categoryList) {
            //2.1 将查询出来的categoryList集合进行判断 筛选出指定parentId的category对象
            if (category.getParentId().equals(parentId)){
                HashMap map = new HashMap();
                //2.2将查询出来的category对象按照需要的数据格式进行封装
