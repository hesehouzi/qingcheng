...
public class BrandServiceImpl implements BrandService {
    ...
    public void saveAllBrandToRedis() {
            ...
            //1.2再遍历categoryList 通过findListByCategoryName方法查询获得brandList 并存入缓存
            for (Category category : categoryList) {
                List<Map> brandList = brandMapper.findListByCategoryName(category.getName());
                redisTemplate.boundHashOps(CacheKey.BRAND).put(category.getName(), brandList);
            }
    }
     ...
     * @return
     */
    public List<Map> findBrandFromRedis(String categoryName) {
        List<Map> brandList = (List<Map>) redisTemplate.boundHashOps(CacheKey.BRAND).get(categoryName);
        return brandList;
    }
